...
<div id="titleImages">
	<img
		src="${pageContext.request.contextPath}/images/board/teamBoard.png"
...
<div id="boardSpace">
	<div id="fix">
		<div id="btn">
			<ul>
				<li class="freeBoard_select"
					onclick="location.href='${pageContext.request.contextPath}/board'"><p>자유게시판</p></li>
				<li class="teamBoard_select active"><p>일행구하기</p></li>
				<li class="notice_select"
					onclick="location.href='${pageContext.request.contextPath}/board/notice'"><p>공지사항</p></li>
			</ul>
		</div>
		<div class="clear"></div>
		...
		<c:if test="${loginMember.memberNick !=null}">
			<div id="writeBtn">
				<a class="btn btn-default pull-right"
					onclick="location.href='${pageContext.request.contextPath}/board/teamBoard_write'">글쓰기</a>
			</div>
		</c:if>
		<div id="pageNum" class="text-center"></div>
...
<script type="text/javascript">
	...
	function boardListDisplay(pageNum) {
		...
		$
				.ajax({
					type : "get",
					url : "${pageContext.request.contextPath}/board/teamBoard_list?pageNum="
							+ pageNum,
					dataType : "json",
					success : function(json) {
						if (json.teamBoardList.length == 0) {
	...
	function pageNumDisplay(pager) {
		...
		for (i = pager.startPage; i <= pager.endPage; i++) {
			if (pager.pageNum != i) {
				html += "<li><a href='javascript:boardListDisplay(" + i
						+ ");'>[" + i + "]</a></li>";
			} else {
				html += "<li><a>" + i + "</a></li>";
			}
